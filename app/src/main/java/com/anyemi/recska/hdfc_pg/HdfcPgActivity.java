...
import com.anyemi.recska.model.PayuResponseModel;
 ...
 */
public class HdfcPgActivity extends AppCompatActivity implements OneClickPaymentListener {
...
// finally change the color
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            window.setStatusBarColor(ContextCompat.getColor(HdfcPgActivity.this, R.color.colorPrimaryDark));
        }
    ...
    private void submitPayment() {
        ...
        new BackgroundTask(HdfcPgActivity.this, new BackgroundThread() {
            @Override
            public Object runTask() {
                return HomeServices.submitPayment(HdfcPgActivity.this, paymentRequestModel);
            }
    ...
    protected void onActivityResult(int requestCode, int resultCode, final Intent data) {
        if (requestCode == PayuConstants.PAYU_REQUEST_CODE) {
            if (data != null) {
                ...
                payuResponseModel = new Gson().fromJson(response, PayuResponseModel.class);

                if (payuResponseModel.getStatus().equals("success")) {
                    paymentRequestModel.setTrsno(payuResponseModel.getTxnid());
                    submitPayment();
                } else {
                    Globals.showToast(getApplicationContext(), payuResponseModel.getError_Message());
                    finish();
                }
...
//                        }).show();
    ...
    public void navigateToBaseActivity(View view) {
         ...
         * should not be used.
         * */
        String salt = "YHqnql1r";
        generateHashFromSDK(mPaymentParams, salt);
    }


    private PaymentParams prepRq() {
...
//        mResponsedata.getStatus().equals("Success")
        if (mResponsedata.getStatus() ==null ) {
            ...
            mPaymentParams.setProductInfo("Bill Payment");
            mPaymentParams.setFirstName(name);
            mPaymentParams.setEmail(email);
            mPaymentParams.setPhone(moile);
    ...
    private class GetHashesFromServerTask extends AsyncTask<String, String, PayuHashes> {
        ...
        protected void onPreExecute() {
            super.onPreExecute();
            progressDialog = new ProgressDialog(HdfcPgActivity.this);
